...
query.parameters <- list(t = movie, plot = "full")
...
movie.data <- fromJSON(body) # put body into key-value pairs

# Make movie_data into a data frame using as.data.frame

movie.data.frame <- as.data.frame(movie.data, stringsAsFactors = FALSE)
...
Director <- function(movie.df) {
  return(paste("Movie:", select(movie.df, Title), "; Director:", select(movie.df, Director)))
}

# Call Director with your favorite movie, and assign it to the variable movie.director

Director(movie.data.frame)
